...
Each fixture directory retains the resolved SwiftTLA and validation-repository SHAs; tool version, jar digest, and Java version; exact commands/options; source and output digests; raw translator and TLC stdout/stderr; raw TLC DOT graphs; canonical graphs; and the exact graph comparison. Evidence directories are fresh and never reused. Canonical graph comparison treats edges as a labeled transition relation: duplicate DOT records with the same source state, action label, and target state count once. The raw DOT stays intact, and each canonical graph records both its raw edge-record count and unique-relation count.

...
PLUSCAL_ORACLE_CHILD_LOG="$log" PLUSCAL_ORACLE_CHILD_RUNNER="$child" \
  "$runner" --case all --checkout "$root" --commit "$commit" --requested-ref main \
...
[ "$actual" = "$expected" ]
[ "$(awk -F '\t' -v checkout="$root" -v commit="$commit" -v corpus="$corpus" '$2 != checkout || $3 != commit || $4 != "main" || $5 != "candidate" || $7 != corpus { print; exit 1 }' "$log")" = "" ]
